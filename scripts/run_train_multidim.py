...
def arg_parse() -> argparse.ArgumentParser.parse_args:
    ...
    parser.add_argument('--config', default='../configs/multidim_unet_aggmasksv2.yaml', type=str, help='load the config file')
...
def run_trainer() -> None:
    ...
    if configs['model'] == 'basic_unet':
        model = BasicUNet(in_channels=3, out_channels=len(selected_organ_labels), dropout=0.1)
